...
class GhostMirror {
  ...
  setupUI() {
    this.rootEl.querySelector('#mirrorConnectBtn').onclick = () => this.connect();
    this.rootEl.querySelector('#mirrorDisconnectBtn').onclick = () => this.disconnect();
    this.rootEl.querySelector('#mirrorSwapBtn').onclick = () => this.toggleSwap();
    this.rootEl.querySelector('#mirrorScaleDown').onclick = () => this.changeScale(-1);
    this.rootEl.querySelector('#mirrorScaleUp').onclick = () => this.changeScale(1);
    ...
    this.keyHandler = (e) => {
      if (!this.rootEl.closest('.ghostos-window--active')) return;
      const keyMap = {
...
function renderGhostOsMirror(rootEl) {
  if (!rootEl) return;
  ...
  if (window.ghostosMirror && typeof window.ghostosMirror.destroy === 'function') {
    window.ghostosMirror.destroy();
  }

  if ('serial' in navigator) {
    rootEl.querySelector('#mirrorUnsupported').style.display = 'none';
    window.ghostosMirror = new GhostMirror(rootEl);
  } else {
...
window.renderGhostOSMirrorApp = renderGhostOsMirror;
