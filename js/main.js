...
function initNonCriticalFeatures() {
  // Initialize AOS with reduced motion preference check (guarded)
  if (
    window.AOS &&
  ...
  // Initialize Feather icons (guarded)
...
// Initialize AOS (guarded for pages that don't load the library)
...
// Initialize Feather icons (guarded for pages that don't load the library)
if (window.feather && typeof feather.replace === "function") {
  feather.replace();
}
...
// Navbar scroll effect (guarded for pages without a navbar)
const navbar = document.querySelector(".navbar");
if (navbar) {
  window.addEventListener("scroll", () => {
...
// (Old inline mobile-menu DOMContentLoaded handler removed; use initMobileMenu instead.)
...
async function fetchLatestRelease() {
  ...
  try {
    ...
    let releaseBodyHtml = "";
    if (window.marked && typeof marked.parse === "function" && release.body) {
      releaseBodyHtml = marked.parse(release.body);
    } else if (release.body) {
      releaseBodyHtml = `<pre>${release.body}</pre>`;
    }

    const assets = Array.isArray(release.assets) ? release.assets : [];
    const totalDownloads = assets.reduce(
      (acc, asset) => acc + (asset.download_count || 0),
      0
    );

    const topAssets = assets.slice(0, 3);
    const extraAssets = assets.slice(3);
      ...
      <div class="release-content">
        <div class="release-info">
          ...
          <div class="release-meta">
            ...
            <span><i data-feather="download"></i>${totalDownloads} downloads</span>
          </div>
          <div class="release-body markdown-content">${releaseBodyHtml}</div>
          <a href="${release.html_url}" class="primary-btn" target="_blank" rel="noopener">
        ...
        <div class="release-assets">
          ...
          <div class="asset-list">
            ${topAssets
              .map(
                (asset) => `
                  <a href="${asset.browser_download_url}" class="asset-link" target="_blank" rel="noopener">
    ...
    if (showMoreBtn) {
      showMoreBtn.addEventListener("click", () => {
        ...
        if (!hiddenAssets || !btnText || !btnIcon) return;

        const isHidden = hiddenAssets.style.display === "none";
        hiddenAssets.style.display = isHidden ? "block" : "none";
        btnText.textContent = isHidden ? "Show Less" : "Show More";
        btnIcon.setAttribute("data-feather", isHidden ? "chevron-up" : "chevron-down");
...
async function fetchFlipperRelease() {
  ...
  try {
    ...
    const installs = (Array.isArray(release.assets)
      ? release.assets
      : []
    ).reduce((acc, asset) => acc + (asset.download_count || 0), 0);

    const shortBody = release.body
      ? release.body.substring(0, 200) + "..."
      : "";
    let notesHtml = shortBody;
    if (window.marked && typeof marked.parse === "function" && shortBody) {
      notesHtml = marked.parse(shortBody);
    } else if (shortBody) {
      notesHtml = `<pre>${shortBody}</pre>`;
    } else {
      notesHtml = "";
    }
    ...
    const metaHTML = `
      <div class="release-meta" data-aos="fade-up">
        ...
        <span><i data-feather="download"></i>${installs} installs</span>
      </div>
    ...
    const assets = Array.isArray(release.assets) ? release.assets : [];
    const fapAsset = assets.find((a) => a.name && a.name.endsWith(".fap"));
    if (fapAsset) {
      const primaryBtn = flipperCard.querySelector(".download-btn.primary-btn");
      if (primaryBtn) {
        primaryBtn.href = fapAsset.browser_download_url;
      }
    }
...
const translations = {
  en: {
    ...
    video3_title: "Dual Communication with GhostESP",
    video3_desc: "How to use dual communication with GhostESP",
    ...
    boards_heading: "Supported Boards",
    boards_intro: "Hand-picked hardware known to work great with Ghost ESP",
    roadmap_heading: "Roadmap",
    roadmap_subheading: "What shipped recently and what's next.",
    feedback_heading: "Submit Feedback",
  },
  de: {
    ...
    feature_network_tools_desc: "DIAL/Chromecast-Steuerung, Netzwerkdrucker-Zugriff und TP-Link-Geräte-Verwaltung",
    feature_security_testing_title: "Sicherheitstests",
    feature_security_testing_desc: "WPS-Scanning, und Skimmer-Erkennung und mehr",
    ...
    video3_title: "Duale Kommunikation mit GhostESP",
    video3_desc: "Wie man duale Kommunikation mit GhostESP verwendet",
    video4_title: "T-Watch S3 Tutorial",
    ...
    boards_heading: "Unterstützte Boards",
    boards_intro: "Sorgfältig ausgewählte Hardware, die hervorragend mit Ghost ESP funktioniert",
    roadmap_heading: "Roadmap",
    roadmap_subheading: "Was kürzlich ausgeliefert wurde und was als Nächstes kommt.",
    feedback_heading: "Feedback senden",
  },
  "in": {
    nav_features: "विशेषताएँ",
    nav_compatibility: "संगतता",
    nav_flash: "फ्लैश",
    nav_get_started: "शुरू करें",
    nav_flipper_app: "फ्लिपर ऐप",
    nav_serial_console: "सीरियल कंसोल",
    nav_merch: "मर्च",
    nav_language: "भाषा",
    hero_get_started: "शुरू करें",
    hero_latest_release: "नवीनतम रिलीज़",
    hero_documentation: "दस्तावेज़ीकरण",
    features_heading: "मुख्य विशेषताएँ",
    feature_wifi_title: "वाई-फाई विश्लेषण",
    feature_wifi_desc: "एक्सेस पॉइंट और स्टेशनों की उन्नत स्कैनिंग, OUI मिलान और रीयल-टाइम मॉनिटरिंग के साथ",
    feature_packet_capture_title: "पैकेट कैप्चर",
    feature_packet_capture_desc: "प्रोब रिक्वेस्ट, बीकन, डीऑथ पैकेट, WPS, EAPOL और कच्चे वाई-फाई पैकेट कैप्चर करें (PCAP सपोर्ट के साथ)",
    feature_beacon_ops_title: "बीकन ऑपरेशन",
    feature_beacon_ops_desc: "नेटवर्क परीक्षण के लिए रैंडम, रिकरोल या AP सूची मोड के साथ कस्टम बीकन फ्रेम जनरेट करें",
    feature_ble_toolkit_title: "BLE टूलकिट",
    feature_ble_toolkit_desc: "फ्लिपर डिवाइस स्कैन करें, BLE स्पैम पहचानें, AirTags ट्रैक करें और कच्चे BLE पैकेट कैप्चर करें",
    feature_wardriving_title: "वारड्राइविंग",
    feature_wardriving_desc: "GPS-सक्षम वाई-फाई और BLE वारड्राइविंग, नेटवर्क मैपिंग के लिए CSV लॉगिंग के साथ",
    feature_evil_portal_title: "ईविल पोर्टल",
    feature_evil_portal_desc: "ऑनलाइन/ऑफ़लाइन मोड और डोमेन स्पूफिंग के साथ कस्टमाइज़ेबल कैप्टिव पोर्टल बनाएं",
    feature_network_tools_title: "नेटवर्क टूल्स",
    feature_network_tools_desc: "DIAL/Chromecast कंट्रोल, नेटवर्क प्रिंटर एक्सेस और TP-Link डिवाइस मैनेजमेंट",
    feature_security_testing_title: "सिक्योरिटी टेस्टिंग",
    feature_security_testing_desc: "WPS स्कैनिंग, स्किमर डिटेक्शन और अन्य सुरक्षा परीक्षण सुविधाएँ",
    feature_display_features_title: "डिस्प्ले फ़ीचर्स",
    feature_display_features_desc: "इन-बिल्ट गेम्स, म्यूज़िक विज़ुअलाइज़र और समर्थित डिस्प्ले बोर्ड्स के लिए RGB रेव मोड",
    video_heading: "Ghost ESP को एक्शन में देखें",
    video1_title: "GhostESP अवलोकन",
    video1_desc: "GhostESP की विशेषताओं का व्यापक अवलोकन, जिसमें WebUI, Evil Portal, Rave मोड और नेटवर्क प्रिंटिंग शामिल हैं",
    video2_title: "डीऑथ डेमो",
    video2_desc: "GhostESP और Flipper Zero द्वारा 2.4GHz वाई-फाई नेटवर्क से स्पाय कैमरा डीऑथेंटिकेट करने का प्रदर्शन",
    video3_title: "GhostESP के साथ डुअल कम्युनिकेशन",
    video3_desc: "GhostESP के साथ डुअल कम्युनिकेशन कैसे उपयोग करें",
    video4_title: "T-Watch S3 ट्यूटोरियल",
    video4_desc: "T-Watch S3 को बूटलोडर मोड में कैसे लाएँ और GhostESP फ्लैश करें",
    video5_title: "मोबाइल फ्लैशिंग ट्यूटोरियल",
    video5_desc: "सिर्फ अपने फोन का उपयोग करके ESP32-C3 SuperMini पर Ghost ESP फ्लैश करें",
    compatibility_heading: "समर्थित हार्डवेयर",
    compatibility_display_heading: "डिस्प्ले-सक्षम बोर्ड",
    compatibility_cyd_title: "CYD बोर्ड",
    compatibility_cyd_note: "2.8\" ESP32-2432S028 के साथ संगत",
    compatibility_7inch_title: "7-इंच डिस्प्ले",
    compatibility_7inch_note: "दोनों ESP32-S3 का उपयोग करते हैं",
    compatibility_other_display_title: "अन्य डिस्प्ले बोर्ड",
    compatibility_generic_heading: "सामान्य बोर्ड",
    compatibility_esp_models_title: "ESP32 मॉडल",
    compatibility_custom_builds_title: "कस्टम बिल्ड",
    compatibility_custom_item1: "अपना खुद का बिल्ड बनाएँ",
    compatibility_custom_item2: "फ़ीचर्स कस्टमाइज़ करें (और भी जल्द!)",
    compatibility_custom_item3: "अपने विशेष हार्डवेयर के लिए बिल्ड करें",
    compatibility_custom_link: "बिल्ड गाइड देखें →",
    getting_started_heading: "शुरुआत करें",
    getting_started_step1_title: "अपना डिवाइस फ्लैश करें",
    getting_started_step1_desc: "Espresso Flash वेब फ्लैशर पर जाएँ",
    getting_started_step1_bootloader: "बूटलोडर मोड:",
    getting_started_step1_boot1: "BOOT बटन दबाकर रखें",
    getting_started_step1_boot2: "USB केबल कनेक्ट करें",
    getting_started_step1_boot3: "कनेक्शन के बाद BOOT छोड़ें",
    getting_started_step2_title: "बेसिक कमांड",
    getting_started_step2_cmd1: "<code>scanap</code> - वाई-फाई नेटवर्क स्कैन करें",
    getting_started_step2_cmd2: "<code>list -a</code> - मिले हुए नेटवर्क की सूची",
    getting_started_step2_cmd3: "<code>scanlocal</code> - लोकल नेटवर्क स्कैन करें",
    getting_started_step2_cmd4: "<code>help</code> - सभी कमांड दिखाएँ",
    getting_started_step3_title: "टच नेविगेशन",
    getting_started_step3_nav1: "ऊपरी हिस्सा: ऊपर जाएँ",
    getting_started_step3_nav2: "निचला हिस्सा: नीचे जाएँ",
    getting_started_step3_nav3: "मध्य भाग: आइटम चुनें",
    getting_started_step3_nav4: "मुख्य मेनू: डायरेक्ट टच",
    getting_started_step4_title: "मदद चाहिए?",
    getting_started_step4_desc: "हमारे Discord समुदाय से जुड़ें:",
    getting_started_step4_help1: "लाइव सपोर्ट",
    getting_started_step4_help2: "समस्या निवारण",
    getting_started_step4_help3: "ताज़ा अपडेट",
    getting_started_step4_help4: "समुदाय टिप्स",
    flipper_heading: "फ्लिपर ज़ीरो ऐप",
    flipper_card_title: "Flipper Zero के लिए Ghost ESP",
    flipper_card_desc: "हमारे आधिकारिक कंपेनियन ऐप के साथ सीधे अपने Flipper Zero से Ghost ESP नियंत्रित करें।",
    flipper_download_fap: "नवीनतम FAP डाउनलोड करें",
    flipper_view_source: "स्रोत देखें",
    flipper_app_features_title: "ऐप फ़ीचर्स:",
    flipper_feature1: "वाई-फाई ऑपरेशन",
    flipper_feature2: "BLE नियंत्रण",
    flipper_feature3: "GPS फ़ंक्शन",
    flipper_feature4: "डिवाइस कॉन्फ़िगरेशन",
    merch_heading: "सीमित आधिकारिक स्टोर",
    merch_desc: "एक्सक्लूसिव Ghost ESP परिधान और एक्सेसरीज़",
    merch_feature1: "उच्च-गुणवत्ता वाला गियर",
    merch_feature2: "विश्व-व्यापी शिपिंग",
    merch_feature3: "सुरक्षित भुगतान",
    merch_visit_store: "स्टोर देखें",
    footer_developed_by: "साइट विकसित: Jay Candel",
    footer_support: "परियोजना का समर्थन करें",
    boards_heading: "समर्थित बोर्ड",
    boards_intro: "Ghost ESP के साथ बेहतरीन काम करने वाला चुना हुआ हार्डवेयर",
    roadmap_heading: "रोडमैप",
    roadmap_subheading: "हाल ही में क्या जारी हुआ और आगे क्या आने वाला है।",
    feedback_heading: "फीडबैक भेजें",
  },
};

const languageToggleButton = document.getElementById("language-toggle-btn");
const languageDropdown = document.getElementById("language-dropdown");
const languageButtons = languageDropdown
  ? languageDropdown.querySelectorAll("button[data-lang]")
  : [];
...
function setLanguage(lang) {
  ...
  if (languageDropdown && languageToggleButton) {
...
if (languageToggleButton && languageDropdown) {
  languageToggleButton.addEventListener("click", (event) => {
    ...
    languageToggleButton.classList.toggle("open");
  });
...
// Ensure German and Indian ('in') have fallbacks for any missing keys
Object.keys(translations.en).forEach((key) => {
  if (!translations.de[key]) translations.de[key] = translations.en[key];
  if (!translations["in"][key]) translations["in"][key] = translations.en[key];
});
